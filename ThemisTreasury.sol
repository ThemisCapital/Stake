...
contract ThemisTreasury is Ownable {
    ...
    address public sTHS;
    uint256 public sTHSQueue; // Delays change to sTHS address
    ...
    function deposit(
        ...
        IERC20Mintable(THS).mint(msg.sender, send);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOfToken(_token, _amount);
        ITHSERC20(THS).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sTHS).balanceOf(msg.sender); // Can only borrow against sTHS held
    ...
    function repayDebtWithTHS(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        ITHSERC20(THS).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, THS, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(THS).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(THS).totalSupply().sub(totalDebt));
    }
    ...
    {
        if (isReserveToken[_token]) {
            value_ = _amount.mul(10**IERC20(THS).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.STHS) {
            // 9
            sTHSQueue = block.number.add(blocksNeededForQueue);
        } else return false;
